...
if __name__ == "__main__":
    ...
    # Rate requirements in asymmetric case
    # rate_req_t = 6
    # rate_req_r = 1
    rate_requirements=[0.45,0.46,0.48,0.5,0.52,0.53,0.54]
    power_vs_rate_oma=[]
    power_vs_rate_noma=[]
    ...
    # _, fitness_history_noma = genetic_algorithm_noma(population_size, generations, 16)
    
    # plt.figure(figsize=(10, 6))
    # plt.plot(range(1, len(fitness_history_noma) + 1), fitness_history_noma, marker="o", linestyle="-", color="g", label="NOMA")
    # plt.xlabel("Generation", fontsize=14)
    # plt.ylabel("Best Fitness Value", fontsize=14)
    # plt.title("Fitness Value vs Generation for NOMA", fontsize=16)
    # plt.grid(True, linestyle="--", alpha=0.7)
    # plt.legend()
    # plt.tight_layout()
    # plt.show()

    rate_req=[x*18 for x in rate_requirements]
   
    # Plotting Power vs Rate Requirements
    plt.figure(figsize=(8, 6)) 
    # OMA
    plt.plot(rate_req, power_vs_rate_oma, 
            marker="o", linestyle="-", color="g", label="Avg Power Consumption for OMA")
    # NOMA
    plt.plot(rate_req, power_vs_rate_noma, 
            marker="o", linestyle="-", color="b", label="Avg Power Consumption for NOMA")

    plt.xlabel("Rate Requirements for strong user(bit/s/Hz)", fontsize=14)
    ...
    # Plotting Power vs Rate Requirements (only NOMA case)
    plt.figure(figsize=(8, 6)) 
    # NOMA
    plt.plot(rate_req, power_vs_rate_noma, 
            marker="o", linestyle="-", color="b", label="Avg Power Consumption for NOMA")
    ...
    plt.xlabel("Rate Requirements for strong user(bit/s/Hz)", fontsize=14)
    plt.ylabel("Average Power Consumption (Watts)", fontsize=14)
    plt.title("Power Consumption vs Rate Requirements (NOMA)", fontsize=16)
